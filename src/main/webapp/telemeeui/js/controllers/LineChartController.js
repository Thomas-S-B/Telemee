...
angular.module('myApp.GraphLineChart', [])
        .controller('MyGraphLineChartCtrl', function($scope, $timeout, Configuration, GraphSize, LogValue, TelemeeApps) {
           //look at https://github.com/novus/nvd3/blob/master/src/models/lineWithFocusChart.js
           ...
           $scope.interpolations = [{name: 'linear'}, {name: 'step-before'}, {name: 'step-after'}, {name: 'basis'}, {name: 'bundle'}, {name: 'cardinal'}, {name: 'monotone'}];
           $scope.interpolation = $scope.interpolations[0];
...
// TODO whatabout non numeric xAttr?
                    ...
                    if ($scope.init) {
                       ...
                       nv.addGraph(function() {
                          ...
                          $scope.chart.yAxis.axisLabelDistance(30);
                          $scope.chart.yAxis.axisLabel(yName);
                          $scope.chart.interpolate($scope.interpolation.name);
                          $scope.chart.xAxis.rotateLabels(10);
                          $scope.chart.margin2({top: 40, right: 30, bottom: 30, left: 60}); //focuschart margins
                          $scope.chart.height2(130);   //Focus height
                          $scope.chart.noData("Waiting for data from " + channel.name);
                          $scope.chart.x2Axis.rotateLabels(10);
                          $scope.chart.transitionDuration(1);   //for mainchart
